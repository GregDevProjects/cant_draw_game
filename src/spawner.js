...
import PizzaCar from './actors/pizza_car'
...
export default class Spawner {
  constructor ( scene ) {
    ...
    this.pizza_group = this.scene.add.group( {
      classType: PizzaCar,
      maxSize: 20,
      runChildUpdate: true
    } )
  ...
  update () {
    this.spawn_test.forEach( ( item, index, object ) => {
      if ( item.y > this.player.y - player_top_distance ) {
        ...
        } else if ( item.type === "pizza_car" ) {
          actor = this.pizza_group.get()
        }

        if ( !actor ) {
          console.warn( 'couldn\'t spawn ' + item.type + ' because it exceeds pool size' )
